...
class TrendReq(object):
    ...
    def _get_data(self, url, method=GET_METHOD, trim_chars=0, **kwargs):
        ...
        if self.proxies != '':
            s.proxies.update(self.proxies[json.loads(self.proxy_counter)]['proxy'])
        try:
            if method == TrendReq.POST_METHOD:
                response = s.post(url, cookies=self.cookies, proxies=self.proxies[json.loads(self.proxy_counter)]['proxy'], **kwargs)
            else:
                response = s.get(url, cookies=self.cookies, proxies=self.proxies[json.loads(self.proxy_counter)]['proxy'], **kwargs)
        except requests.exceptions.ProxyError:
            print('Proxy {} error. Swiching to proxy {}'.format(self.proxies[json.loads(self.proxy_counter)]['proxy'], self.proxies[json.loads(self.proxy_counter)]['proxy']))
